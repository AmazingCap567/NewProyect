...
class Clientes(tk.Frame):
    ...
    def modificar(self):
        if not self.tre.selection():
            messagebox.showerror("Error", "Por favor selecciones un cliente para modificar")
            return
        ...
        def guardar_modificaciones():
            ...
            try:
                ...
                cursor.execute("""UPDATE clientes SET nombre = ?, dni = ?, celular = ?, direccion = ?, correo = ? WHERE id = ?""",
                               (nuevo_nombre, nuevo_dni, nuevo_celular, nuevo_direccion, nuevo_correo, id_cliente))
                conn.commit()
                conn.close()
