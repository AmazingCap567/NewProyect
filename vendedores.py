...
class Vendedores(tk.Frame):
    ...
    def widgets(self):
        self.labelframe = tk.LabelFrame(self, text="Vendedores", font="sans 20 bold", bg="#C6D9E3")
        ...
        lbldni = tk.Label(self.labelframe, text="DNI: ", font="sans 14 bold", bg="#C6D9E3")
    ...
    def registrar(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("INSERT INTO vendedores (nombre, dni, celular, direccion, correo) VALUES(?,?,?,?,?)", 
            ...
            messagebox.showerror("Error", f"No se pudo registrar el vendedor: {e}")
    ...
    def modificar(self):
        if not self.tre.selection():
            messagebox.showerror("Error", "Por favor seleccione un vendedor para modificar")
            return
        
        item = self.tre.selection()[0]
        id_vendedor = self.tre.item(item, "values")[0]
        ...
        top_modificar.title("Modificar vendedor")
        ...
        def guardar_modificaciones():
            ...
            try:
                conn = sqlite3.connect(self.db_name)
                cursor = conn.cursor()
                cursor.execute("""UPDATE vendedores SET nombre = ?, dni = ?, celular = ?, direccion = ?, correo = ? WHERE id = ?""",
                               (nuevo_nombre, nuevo_dni, nuevo_celular, nuevo_direccion, nuevo_correo, id_vendedor))
                conn.commit()
                conn.close()
                messagebox.showinfo("Exito", "Vendedor modificado correctamente")
                self.limpiar_treeview()
                self.cargar_registros()
